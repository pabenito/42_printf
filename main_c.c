...
/*   Updated: 2024/03/22 18:13:36 by pbenito-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	char	input[SIZE] = {'a', 'A', 'z', 'Z', ' ', '#', '\n'};
	char	*format = "Print this character: %c";
	int		result;
	int		expected;
	int		i;

	printf("Function: ft_printf %%c\n");
	i = 0;
	while (i < SIZE)
	{
		...
		if (result == expected)
			...
			set_red();
		printf("Format: \"%s\"; Input: '%c'; Result: %d; Expected: %d\n", format, input[i], result, expected);
		fflush(stdout);
		reset_color();
		i++;
	}
